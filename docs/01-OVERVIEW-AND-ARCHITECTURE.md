...
|---|---|---|
| Log Analyzer | Find error patterns near the incident window | Loki (LogQL) |
| Metrics | Detect SLO violations and correlated metric anomalies | Prometheus (PromQL) |
| Topology | Map affected service to dependencies and recent deploys | Service graph + deploy log |
| History | Find similar past incidents | Postgres + pgvector |
| Runbook | Match incident to a documented playbook | Runbook markdown files |
| Synthesizer | Combine all findings into one report | The other agents' outputs |
